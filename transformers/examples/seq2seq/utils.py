...
class AbstractSeq2SeqDataset(Dataset):
    def __init__(
        ...
        **dataset_kwargs
        ...
        super().__init__()
        self.langs = list(lang_dict.keys())
        self.src_files = {}
        self.tgt_files = {}

        for lang in self.langs:
            self.src_files[lang] = Path(data_dir).joinpath(type_path + ".source" + lang)
            self.tgt_files[lang] = Path(data_dir).joinpath(type_path + ".target" + lang)
        ...
        if os.path.exists(self.len_file):
            ...
            self.src_lens = self.get_char_lens(self.src_files[self.langs[0]])
