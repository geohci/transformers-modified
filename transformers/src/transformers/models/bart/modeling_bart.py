...
class BartModel(PretrainedBartModel):
    def __init__(self, config: BartConfig):
        ...
        self.mapping = Attention(config.d_model, config.decoder_attention_heads, dropout=config.attention_dropout, encoder_decoder_attention=True)
    ...
    def forward(
        ...
        input_ids_fre=None,
        target_lang=None,
        attention_mask_fre=None,
        encoder_outputs_fre: Optional[Tuple] = None,
        input_ids_cs=None,
        attention_mask_cs=None,
        encoder_outputs_cs: Optional[Tuple] = None,
        main_lang=None,
        ...
        if main_lang is None:
            langs = list(attention_mask.keys())
            for key, val in attention_mask.items():
                if val.shape[1] == 1:
                    langs.remove(key)
        ...
        for lang, key in encoder_outputs.items():
            ...
            enc_outputs, att_weight = self.mapping(query=query, key=key, key_padding_mask=mask, output_attentions=output_attentions)
