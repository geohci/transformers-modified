...
class GenerationMixin:
    ...
    #    self, input_ids: torch.Tensor, input_ids_fre: torch.Tensor, pad_token_id: int, eos_token_id: int, target_lang: str
    #) -> torch.LongTensor:
    #    #is_pad_token_in_inputs_ids = (pad_token_id is not None) and (pad_token_id in input_ids)
    #    is_pad_token_in_inputs_ids = True
    #    is_pad_token_not_equal_to_eos_token_id = (eos_token_id is None) or (
    #        (eos_token_id is not None) and (pad_token_id != eos_token_id)
    #    )
    #    if is_pad_token_in_inputs_ids and is_pad_token_not_equal_to_eos_token_id:
    #    #FIXME check the decoder start token id
    #        if (target_lang == "en_XX" and input_ids is not None) or (target_lang == "fr_XX" and input_ids_fre is None):
    #            return input_ids.ne(pad_token_id).long()
    #        else: 
    #            return input_ids_fre.ne(pad_token_id).long()
    ...
    def transform_input_ids(
        self, input_ids: torch.LongTensor, input_ids_fre: torch.LongTensor, pad_token_id: int
    ):
        if input_ids is None:
            input_ids = torch.ones((input_ids_fre.shape[0],1), dtype=torch.long, device=input_ids_fre.device) * pad_token_id
       
        if input_ids_fre is None:
            input_ids_fre = torch.ones((input_ids.shape[0],1), dtype=torch.long, device=input_ids.device) * pad_token_id
            
        return input_ids, input_ids_fre
    ...
    def generate(
        ...
        **model_kwargs
        ...
        if self.config.is_encoder_decoder:
            #input_ids, input_ids_fre = self.transform_input_ids(input_ids, input_ids_fre, pad_token_id)
